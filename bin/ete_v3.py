...
def main(taxname, refs, report, output):
    ...
    with open(report, "r") as r:
        for line in r:
            ...
            if len(elements) > 5:
                ...
                tax = elements[2]   # Hit Taxid
                cons_tax = elements[5] # Consensus Taxid
                bucket[tax] = sci
                bucket[cons_tax] = sci
    ...
    for n in tree.traverse(strategy="postorder"):
        ...
        # Determine presence in consensus report (bucket) or blast DB directly (no bubbling up)
        n.is_in_bucket = (tid in bucket)
        n.is_in_db = (tid in blast_tax)
    ...
    rank_order = {
        "forma": 0, "varietas": 1, "subspecies": 2, "species": 3,
        "genus": 4, "family": 5, "order": 6, "class": 7,
        "phylum": 8, "kingdom": 9, "no rank": 10
    }

    # Sort data by rank order, then alphabetically by taxon name
    sorted_taxa = sorted(
        data.keys(),
        key=lambda k: (
            rank_order.get(data[k].split("\t")[-1].strip(), 99),
            k
        )
    )
